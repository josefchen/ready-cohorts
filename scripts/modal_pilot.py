...
REMOTE_CONFIG = REMOTE_ROOT / "configs/pilot-002-modal-l4.toml"
REMOTE_OUTPUT = Path("/tmp/benchmark-output")

image = (
    modal.Image.debian_slim(python_version="3.12")
    .pip_install("torch==2.13.0", "pandas==3.0.5")
    .add_local_dir(REPO_ROOT / "src", str(REMOTE_ROOT / "src"), copy=True)
    .add_local_file(
        REPO_ROOT / "configs/pilot-002-modal-l4.toml",
        str(REMOTE_CONFIG),
        copy=True,
    )
)

app = modal.App("gpu-agent-crossover-pilot-002", image=image)


@app.function(gpu="L4", cpu=8.0, timeout=30 * 60, single_use_containers=True)
def run_l4_pilot() -> dict[str, str]:
    sys.path.insert(0, str(REMOTE_ROOT / "src"))
    from gpu_agent_crossover.benchmark import run
